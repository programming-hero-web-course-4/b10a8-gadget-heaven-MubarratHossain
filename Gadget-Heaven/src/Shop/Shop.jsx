...
const Shop = () => {
    // Fetching the loader data
    const data = useLoaderData();
    const categories = data.categories; // Extract categories from the loaded data
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            {categories.map((category) => (
                <div key={category.category_name} className="mb-6">
                    <h4 className="text-2xl font-semibold mb-2">{category.category_name}</h4>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {category.items.map((item) => (
                            <div key={item.product_id} className="border rounded-lg p-4 shadow-md transition-transform duration-200 hover:scale-105">
                                ...
                                <button className="mt-4 w-full px-4 py-2 bg-[#9538E2] text-white rounded-full hover:bg-[#7a28a5] transition duration-300">
                                    Buy Now
                                </button>
                            </div>
                        ))}
